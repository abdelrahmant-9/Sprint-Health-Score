...
# â”€â”€ Status sets â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Issue type icons â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
ALL_ISSUE_TYPES = {
    "Bug":         ("BUG", "#ff4757"),
    "Story":       ("STORY", "#1a6bff"),
    "Sub-task":    ("SUB", "#4a90d9"),
    "Task":        ("TASK", "#a78bfa"),
    "Enhancement": ("ENH", "#00d4aa"),
    "Improvement": ("ENH", "#00d4aa"),
    "Feature Bug": ("FBUG", "#ff6b35"),
}
DEFAULT_ISSUE_ICON = ("ITEM", "#4a90d9")

# â”€â”€ Scoring weights â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _signal_threshold_texts() -> dict:
    ...
    return {
        "commitment": (
            f"{_format_decimal(commitment['ideal_min_pct'])}-{_format_decimal(commitment['ideal_max_pct'])}% -> {points['excellent']} pts<br>"
            f"{_format_decimal(commitment['good_min_pct'])}-{_format_decimal(commitment['ideal_min_pct']-1)}% -> {points['good']} pts<br>"
            f"{_format_decimal(commitment['warning_min_pct'])}-{_format_decimal(commitment['good_min_pct']-1)}% -> {points['warning']} pts<br>"
            f"&lt;{_format_decimal(commitment['warning_min_pct'])}% -> {points['poor']} pts"
        ),
        "carryover": (
...
# â”€â”€â”€ JIRA CLIENT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ SPRINT STATE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ CHANGELOG & STATUS HISTORY â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ BURNDOWN â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def build_burndown(issues: list, ss: SprintState) -> dict:
    ...
    return {
        "total_issues": total_issues, "total_days": total_days,
...
def score_carryover(carried, total, sprint_pct=None, is_extended=False):
    ...
    score = _blend(raw, sprint_pct, points["neutral"])
    if is_extended: score = max(0, score - int(cfg["extended_penalty"]))
    return score, round(pct, 1)
...
# â”€â”€â”€ DEVELOPER & QA ACTIVITY â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def build_developer_activity(
    ...
    """
    Returns:
      dev_activity â€” issues updated today grouped by assignee (or Unassigned)
      qa_activity  â€” all status transitions today
    """
    ...
    for issue in issues:
        ...
        # â”€â”€ Fetch changelog for every sprint issue â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ QA Activity â€” transitions today â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        for t in today_transitions:
            ...
            if to_upper in qa_upper:
                ...
                qa_items.append(_qa_event(
                    ...
                    f"â†” {t['from']} â†’ {t['to']}", "#4a90d9", time_in_rft, url, story_points
                ))

        # â”€â”€ Developer Activity â€” updated today â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        for t in today_transitions:
            ...
            transition_label = f"{t['from']} â†’ {t['to']}"
...
def build_report(issues: list, sprint_info: dict, prev_sprints: list) -> dict:
    ...
    bug_change_pct, bug_change_arrow = None, "â†’"
    if prev_bugs is not None and prev_bugs > 0:
        bug_change_pct   = round((bugs - prev_bugs) / prev_bugs * 100, 1)
        bug_change_arrow = "â†“" if bug_change_pct < 0 else ("â†‘" if bug_change_pct > 0 else "â†’")
    ...
    return {
        ...
        "signals": {
            "commitment": {"score": c_score, "pct": c_pct, "raw": f"{done}/{total} issues done", "no_data": total == 0},
            "carryover":  {"score": co_score, "pct": co_pct, "raw": f"{carried_over}/{total} carried over", "no_data": total == 0},
            "cycle_time": {
        ...
        "formula": (
            f"({c_score}Ã—0.35) + ({co_score}Ã—0.25) + ({cy_score}Ã—0.20) + ({b_score}Ã—0.20)"
...
def _build_burndown_svg(bd: dict) -> str:
    ...
    W, H   = 620, 260
    PAD_L, PAD_R, PAD_T, PAD_B = 48, 20, 20, 44
    ...
    return f"""<svg viewBox="0 0 {W} {H}" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:{W}px;height:auto;display:block">
  {grid_lines}
...
def _issue_row_html(iss: dict, show_rft: bool = True) -> str:
    ...
    stale_tag   = (
        f'<span class="issue-stale-tag">ðŸ”´ Stale ({iss["active_days"]}d / {iss["stale_threshold"]}d)</span>'
    ...
    done_tag = '<span class="issue-done-tag">âœ“ Done</span>' if iss.get("is_done") else ""
    rft_tag  = (
        f'<span class="issue-active-tag">ðŸ• {iss["time_in_rft"]}d in testing</span>'
    ...
    <div class="dev-issue {'stale' if iss.get('is_stale') else ''}">
      ...
      <div class="issue-body">
        ...
        <span class="issue-summary" style="{done_style}">{escape(iss['summary'][:70])}{'â€¦' if len(iss['summary'])>70 else ''}</span>
    ...
    for dev in sorted_devs:
        ...
        for iss in issues:
            ...
            <div class="dev-issue {'stale' if iss.get('is_stale') else ''}">
              ...
              <div class="issue-body">
                <a href="{iss['url']}" target="_blank" class="issue-key">{iss['key']}</a>
                <span class="issue-summary">{escape(iss['summary'][:90])}{'â€¦' if len(iss['summary']) > 90 else ''}</span>
...
def format_slack_message(r: dict) -> str:
    ...
    health_dot = "ðŸŸ¢" if score >= 85 else "ðŸŸ¡" if score >= 70 else "ðŸŸ " if score >= 50 else "ðŸ”´"
    filled     = round(score / 10)
    bar        = "â–ˆ" * filled + "â–‘" * (10 - filled)

    def sig_dot(s): return "ðŸŸ¢" if s >= 85 else "ðŸŸ¡" if s >= 70 else "ðŸŸ " if s >= 50 else "ðŸ”´"
    def nd(k): return " _â€” no data yet_" if r["signals"][k].get("no_data") else ""
    ...
    sig_rows = (
        f"{sig_dot(sigs['commitment']['score'])}  *Commitment*  {sigs['commitment']['raw']}  â†’  *{sigs['commitment']['score']} pts*{nd('commitment')}\n"
        f"{sig_dot(sigs['carryover']['score'])}  *Carryover*   {sigs['carryover']['raw']}  â†’  *{sigs['carryover']['score']} pts*{nd('carryover')}\n"
        f"{sig_dot(sigs['cycle_time']['score'])}  *Cycle Time*  {sigs['cycle_time']['raw']}  â†’  *{sigs['cycle_time']['score']} pts*{nd('cycle_time')}\n"
        f"{sig_dot(sigs['bug_ratio']['score'])}  *Bug Ratio*   {sigs['bug_ratio']['raw']}  â†’  *{sigs['bug_ratio']['score']} pts*{nd('bug_ratio')}\n"
        f"ðŸ›  *New Bugs*  {r['new_bugs']} created ({r['new_bugs_done']} resolved)   |   ðŸ“¦ *Carried* {r['carried_bugs']}"
    )

    bd = r.get("burndown", {})
    bd_line = ""
    if bd:
        track_icon = "âœ…" if bd.get("on_track") else ("âš ï¸" if not bd.get("is_extended") else "ðŸ”´")
        ext_note   = " _(sprint overran)_" if bd.get("is_extended") else ""
        bd_line    = (
            f"\n*Burndown*  Day {bd['elapsed_days']}/{bd['total_days']}  Â·  "
            f"{bd['current_remaining']} remaining  Â·  Ideal: {bd['ideal_remaining']}  Â·  "
            f"{track_icon} {'On track' if bd.get('on_track') else 'Behind'}{ext_note}  Â·  "
            f"Velocity: {bd['velocity']}/day  Â·  Projected: {bd['projected_end']}\n"
        )
    ...
    status_lines = "\n".join(
        f"  â€¢ {k}: {v}" for k, v in sorted(r["status_counts"].items(), key=lambda x: -x[1])
    ) or "  â€¢ No issues found"

    no_data_note   = "\n> â„¹ï¸ _No issues yet â€” neutral score of 70 used._\n" if r["no_data_signals"] else ""
    state_banner   = ""
    if r["sprint_state"] == "extended":
        state_banner = "\n> âš ï¸ _Sprint passed end date â€” not yet closed._\n"
    elif r["sprint_state"] == "closed":
        state_banner = "\n> ðŸ“‹ _Showing last closed sprint._\n"

    date_range    = f"{r['sprint_start']} â†’ {r['sprint_end']}" if r["sprint_start"] and r["sprint_end"] else "Dates not set"
    progress_note = f"   Â·   Day {r.get('elapsed_days','?')}/{r.get('total_days','?')} ({r['sprint_progress_pct']}%)" if r.get("sprint_progress_pct") is not None else ""
    ...
    if r.get("dev_activity"):
        ...
        for dev in r["dev_activity"]:
            ...
            stale_note  = f" âš ï¸ {stale_count} stale" if stale_count else ""
            dev_lines  += f"  ðŸ‘¤ *{dev['name']}* â€” {len(dev['issues'])} issue(s){stale_note}\n"
            for iss in dev["issues"]:
                icon, _ = ALL_ISSUE_TYPES.get(iss["type"], DEFAULT_ISSUE_ICON)
                stale_tag  = " ðŸ”´ _stale_" if iss["is_stale"] else ""
                active_tag = f" _(active {iss['active_days']}d)_" if iss["active_days"] > 1 else ""
                rft_tag    = f" _(ðŸ• {iss['time_in_rft']}d testing)_" if iss.get("time_in_rft", 0) > 0 else ""
                dev_lines += f"    {icon} {iss['key']} Â· {iss['status']}{active_tag}{rft_tag}{stale_tag}\n"
    ...
    if r.get("qa_activity"):
        ...
        for item in r["qa_activity"]:
            ...
            rft_tag  = f" _(ðŸ• {item['time_in_rft']}d)_" if item.get("time_in_rft", 0) > 0 else ""
            qa_lines += f"  {icon} *{item['key']}* {item['label']}{rft_tag} Â· {item['summary'][:50]}\n"

    return (
        f"ðŸ“Š  *Sprint Health Report*  â€”  Lumofy QA\n"
        f"*{r['sprint_name']}*   Â·   {date_range}{progress_note}\n"
        f"{'â”€' * 44}\n\n"
        ...
        f"ðŸ› Bugs: *{r['bugs']}*   |   ðŸ“¦ Scope: *{r['total']}*   |   ðŸš§ Blockers: *{r['blocked_count']}*\n\n"
        f"_Generated {r['generated_at']}  Â·  Lumofy QA Dashboard_"
    )
...
def format_slack_site_message(r: dict, site_url: str, pdf_url: str = "") -> str:
    ...
    bd      = r.get("burndown", {})
    bd_note = f"\nBurndown: {bd['current_remaining']} remaining Â· {'âœ… On track' if bd.get('on_track') else 'âš ï¸ Behind'}" if bd else ""
    return (
        f"ðŸš€ Sprint Health Report Ready â€” Lumofy QA\n\nScore: {score}/100 {health_dot}\n"
        f"{bugs_line}\nCycle Time: {cycle_time}{bd_note}\n\nðŸ”— View Report:\n{site_url}"
    )


# â”€â”€â”€ HTML REPORT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def write_html_report(r: dict, output_path: str = "sprint_health_report.html") -> str:
    ...
    def nd_badge(k):
        return '<span class="no-data-badge">no data â€” neutral</span>' if r["signals"][k].get("no_data") else ""
    ...
    signal_defs = [
        ...
        {"key": "bug_ratio",  "label": "Bug Ratio (New Only)", "score": sigs["bug_ratio"]["score"],
         ...
         "formula": "New Bugs (created this sprint) Ã· Total Issues"},
    ]
    signals_html = ""
    for sd in signal_defs:
        ...
        <div class="signal-card">
          ...
          <div class="signal-metric">{sd['metric']} &nbsp;Â·&nbsp; {sd['pct']}%</div>
          {nd_badge(sd['key'])}
          <div class="signal-explanation">
            <div class="explanation-title">How it's calculated:</div>
            {sd['formula']}<br><strong>Scoring:</strong><br>{thresholds[sd['key']]}
          </div>
        </div>"""
...
.signal-card{{background:rgba(10,20,40,.9);border-radius:14px;padding:22px 16px;
  border:1px solid rgba(26,107,255,.2);text-align:center;transition:transform .25s,border-color .25s}}
.signal-card:hover{{transform:translateY(-4px);border-color:#1a6bff}}
.signal-label{{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:#4a90d9;margin-bottom:10px}}
.signal-score{{font-size:44px;font-weight:900;margin-bottom:6px;display:flex;align-items:baseline;justify-content:center;gap:3px}}
...
<div class="container">
  ...
  <div class="card">{burndown_stats}{burndown_svg}</div>
...
def write_pdf_report(r: dict, output_path: str | None = None) -> str | None:
    ...
    bd   = r.get("burndown", {})
    lines = [
        "Lumofy â€” Sprint Health Report",
        f"Sprint: {r['sprint_name']}",
        f"Dates:  {r['sprint_start']} â†’ {r['sprint_end']}",
        f"State:  {r['sprint_state'].upper()}",
        "",
        f"Health Score: {r['health_score']}/100  â€”  {r['health_label']}",
        "",
        "Signals:",
        f"  Commitment:  {r['signals']['commitment']['raw']}  â†’ {r['signals']['commitment']['score']} pts",
        f"  Carryover:   {r['signals']['carryover']['raw']}   â†’ {r['signals']['carryover']['score']} pts",
        f"  Cycle Time:  {r['signals']['cycle_time']['raw']}  â†’ {r['signals']['cycle_time']['score']} pts",
        f"  Bug Ratio:   {r['signals']['bug_ratio']['raw']}   â†’ {r['signals']['bug_ratio']['score']} pts",
        "",
        "Bug Breakdown:",
...
# â”€â”€â”€ SLACK SEND â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def run(dry_run=False, export_html=False, export_pdf=False, no_slack=False,
    ...
    ss = SprintState(sprint_info)
    print(f"      â†’ {len(issues)} issues in '{ss.name}' [{ss.state}]")

    print("[2/3] Fetching last 3 closed sprints...")
    prev_sprints = fetch_last_n_sprints(n=3)
    print(f"      â†’ {len(prev_sprints)} closed sprints found")
    ...
    print(f"[done] {report['health_score']}/100 â€” {report['health_label']}")
